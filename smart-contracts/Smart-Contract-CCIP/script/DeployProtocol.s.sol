...
import { DeployerNewChainWithCalls } from "../src/newChain/DeployerNewChainWithCalls.sol";
...
contract DeployProtocol is Script {
    ...
    function run() public {
        ...
        DeployerOriginalChain  deployerOriginalChain = new DeployerOriginalChain(
            adminAddress,
            routerCCIP,
            linkTokenAddress
        );

        DeployerNewChain deployerNewChain = new DeployerNewChain(
            adminAddress,
            routerCCIP,
            linkTokenAddress
        );
        
        console2.log("deployer original chain address: ", address(deployerOriginalChain));
        console2.log("deployer new chain address: ", address(deployerNewChain));
    }
}
